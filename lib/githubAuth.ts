...
const GITHUB_SESSION_COOKIE = "agentforge_github_session"
const GITHUB_OAUTH_STATE_COOKIE = "agentforge_github_oauth_state"
...
export async function clearGitHubSession() {
    const store = await cookies()
    store.set(GITHUB_SESSION_COOKIE, "", {
        httpOnly: true,
        sameSite: "lax",
        secure: process.env.NODE_ENV === "production",
        path: "/",
...
export async function readGitHubSession() {
    const store = await cookies()
    const raw = store.get(GITHUB_SESSION_COOKIE)?.value
    if (!raw) return null
...
export async function consumeOAuthState(expected: string) {
    const store = await cookies()
    const actual = store.get(GITHUB_OAUTH_STATE_COOKIE)?.value
    store.set(GITHUB_OAUTH_STATE_COOKIE, "", {
        httpOnly: true,
        sameSite: "lax",
        secure: process.env.NODE_ENV === "production",
        path: "/",
